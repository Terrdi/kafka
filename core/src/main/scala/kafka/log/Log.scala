...
//    initializeLeaderEpochCache()
  ...
  def appendAsLeader(records: MemoryRecords,
                     ...
                     requestLocal: RequestLocal = RequestLocal.NoCaching): LogAppendInfo = { // 写Leader副本
  ...
  private def deletableSegments(predicate: (LogSegment, Option[LogSegment]) => Boolean): Iterable[LogSegment] = {
    if (segments.isEmpty) { // 如果当前日志压根没有任何日志段对象，直接返回
      ...
      while (segmentOpt.isDefined) {
        ...
        if (highWatermark >= upperBoundOffset && predicate(segment, nextSegmentOpt) && !isLastSegmentAndEmpty) { // 同时不满足上述三个条件
          deletable += segment
