...
/**
 * Controller 向 Broker 发送请求所用的通道管理器
 * 管理 Controller 与 集群Broker 之间的连接，并为每个 Broker 创建 RequestSendThread 线程实例
...
class ControllerChannelManager(controllerContext: ControllerContext,
  ...
  /**
   * Key是Integer类型=> Broker的ID信息
   * Value是一个 {@link ControllerBrokerStateInfo}
   */
  ...
  /**
   * Controller 在启动时，会调用 ControllerChannelManager 的 startup 方法。
   * 该方法会从元数据信息中找到对应集群的Broker列表，然后依次为它们调用 addNewBroker 方法，把它们加到 brokerStateInfo 变量中，
   * 最后在依次启动 brokerStateInfo 中的 RequestSendThread 线程
   */
  def startup() = {
  ...
  /**
   * 关闭所有 RequestSendThread线程 并清空必要的资源
   */
  def shutdown() = {
    brokerLock synchronized {
  ...
  /**
   * 添加目标 Broker 到 brokerStateInfo 数据结构中，并创建必要的配套资源，如请求队列、RequestSendThread对象等
  ...
  def addBroker(broker: Broker): Unit = {
    ...
    brokerLock synchronized {
      ...
      if (!brokerStateInfo.contains(broker.id)) {
        // 将新 Broker 加入到 Controller 管理，并创建对应的 RequestSendThread 线程
        addNewBroker(broker)
  ...
  /**
   * 为目标 broker 创建一些了配套资源，如：
   * NetworkClient 网络I/O操作
   * messageQueue 阻塞队列
   * requestThread 用于发送请求
   * @param broker
   */
  private def addNewBroker(broker: Broker): Unit = {
    ...
    val (networkClient, reconfigurableChannelBuilder) = {
      ...
      // 创建 NetWorkClient 实例
      // NetWorkClient 类是 Kafka clients 工程封装的顶层网络客户端 API
      // 提供了丰富的方法实现网络 IO 数据传输
    ...
    // 为这个 RequestSendThread 线程设置线程名称
    ...
    // 创建该Broker专属的 ControllerBrokerStateInfo 实例
    // 并将其放入到 brokerStateInfo 统一管理
  ...
  /**
   * 根据给定的 Broker 序号信息，从 brokerStateInfo 找出对应的 ControllerBrokerStateInfo 对象。
   * 有了这个对象，也就有了该目标 Broker 服务的所有配套资源。从 ControllerBrokerStateInfo 拿出 RequestSendThread 对象并启动。
   * @param brokerId
   */
 ...
 * Controller 阻塞队列
 * @param apiKey
 * @param request 核心字段。阻塞队列上 AbstractControlRequest. <\: 上边界. 即字段 request 必须是
 *                {@link AbstractRequest} 的子类
 * @param callback
 * @param enqueueTimeMs
 */
 ...
 * @param controllerContext Controller 元数据信息
 * @param queue 请求阻塞队列
 * @param networkClient 用于执行发送的网络I/O类
 * @param brokerNode 目标 Broker 节点
  ...
  override def doWork(): Unit = {
    ...
    try {
      ...
      while (isRunning && !isSendSuccessful) {
        ...
        // 如果没有创建与目标 Broker 的 TCP 连接，或连接暂时不可用
        try {
          if (!brokerReady()) {
      ...
      if (clientResponse != null) {
        ...
        // 此 Response 的请求类型必须是 LeaderAndIsrRequest、 StopReplicaRequest或UpdateMetadataRequest中的一种
 ...
 * 承载若干信息的POJO类
 * @param networkClient
 * @param brokerNode 目标 Broker 节点对象，里面封装了目标 Broker 连接信息，比如主机名、端口号等
 * @param messageQueue 请求消息阻塞队列。 Controller 为每个目标 Broker 都创建了一个消息队列
 * @param requestSendThread Controller 使用该线程给目标Broker发送请求
